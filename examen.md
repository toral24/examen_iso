...
```
storal@A201_PC02:~$ echo sergio > /tmp/ev3ex1.txt
```
...
```
sed -nE "/([:digit:]|[:punch:])*[:alpha:]$/p"
```

**b)** Una matrícula de coche (formato nuevo y antiguo). Ejemplos: 2288FSF, 4441KSD, LE3308G, A1234AJ, …

```
sed "/([1-9]{4}[A-Z]{3}|[A-Z]{1,2}[1-9]{4}[A-Z]{1,2})/p"
```

**c)** Una fecha de la forma 04/05/2021

```
sed "/[1-9]{2}\/[1-9]{2}\/[1-9]{4}/p"
```

**d)** Una dirección MAC. Ejemplos: 00-09-0F-FE-00-01, 3C:A0:67:43:D3:92, …

```
sed "/([1-9]|[ABCDEF]){2}[:punch:]([1-9]|[ABCDEF]){2}[:punch:]([1-9]|[ABCDEF]){2}[:punch:]([1-9]|[ABCDEF]){2}[:punch:]([1-9]|[ABCDEF]){2}[:punch:]([1-9]|[ABCDEF]){2}[:punch:]/p"
```

**e)** Una dirección IP. Ejemplos: 192.168.1.31, 10.0.0.1, 172.255.255.1, …

```
sed "/[1-9]{1,2,3}[:punch:][1-9]{1,2,3}[:punch:][1-9]{1,2,3}[:punch:][1-9]{1,2,3}[:punch:]/p"
```
...
```
sed -E "1s/.*/\<usuario\>/" -e "s/(.*),(.*),(.*),/\<nombre\>(1/) (2/) (3/)\<\/nombre\>"
```
